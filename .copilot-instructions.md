...
- **Audience**: Developers consuming paved roads and implementation guidance.
- **Scope**: Focus on services used and the basic flow (e.g., GitHub → CodePipeline/CodeBuild → ECR/ECS or S3/CloudFront → Cloudflare), environment separation (deployments vs workloads accounts), and operational considerations.
- **Exclude deep infrastructure internals**: Do not document Terraform module internals, detailed IAM/VPC/Security Group configurations, or account provisioning steps. Assume CloudOps provisions infrastructure using standard Terraform modules/templates.
- **Samples**: Provide minimal, practical samples (e.g., `buildspec.yml`, `Dockerfile` for containerized apps; S3 sync buildspec for static apps). Avoid unnecessary implementation detail.
- **Diagrams**: Prefer simple architecture hierarchy diagrams that illustrate components and relationships, not low-level configs.
